...
func toManhattenDistances(positions []intersection) []int {
	distances := []int{}
	for _, pos := range positions {
		distances = append(distances, abs(pos.x)+abs(pos.y))
	}
	return distances
}

func toPathLengths(intersections []intersection) []int {
	pathLengths := []int{}
	for _, intersection := range intersections {
		pathLengths = append(pathLengths, intersection.pathLength)
	}
	return pathLengths
}
...
func (p path) intersections(other path) []intersection {
	intersections := []intersection{}
	for p1Dist, p1 := range p {
		for p2Dist, p2 := range other {
			if p1.x == p2.x && p1.y == p2.y {
				intersections = append(intersections, intersection{x: p1.x, y: p1.y, pathLength: p1Dist + p2Dist})
			}
		}
	}
...
func createPath(input string) path {
	path := path{}
	parts := strings.Split(input, ",")
	path = append(path, position{x: 0, y: 0})
	for _, part := range parts {
		dir := string(part[:1])
		value, _ := strconv.Atoi(string(part[1:]))
		for i := 0; i < value; i++ {
			last := path[len(path)-1]
			pos := position{x: last.x, y: last.y}
			if dir == "U" {
				pos.y--
			}
			if dir == "D" {
